...
from PyQt5.QtWidgets import (
    ...
    QStyleOption, QStyle, QCheckBox, QLineEdit
)
from PyQt5.QtCore import QThread, pyqtSignal, Qt, QTimer, QUrl
from PyQt5.QtGui import (
    QKeySequence, QFont, QPainter, QColor, QDesktopServices,
...
THEMES = {
    ...
    "Night Mode": """
        ...
        QTextBrowser, QTextEdit, QLineEdit { background-color: #252526; color: #d4d4d4; border: 1px solid #3e3e42; border-radius: 4px; }
    ...
    "Intelligent Grayscale": """
        ...
        QTextBrowser, QTextEdit, QLineEdit { background-color: #f7f7f7; color: #111111; border: 1px solid #b0b0b0; border-radius: 4px; }
    ...
    "Warm Sepia": """
        ...
        QTextBrowser, QTextEdit, QLineEdit { background-color: #fcf6e8; color: #2e2323; border: 1px solid #d4c5a9; border-radius: 4px; }
    ...
    "High Contrast": """
        ...
        QTextBrowser, QTextEdit, QLineEdit { background-color: #000000; color: #ffffff; border: 2px solid #ffffff; }
...
class ApiWorker(QThread):
    ...
    def __init__(self, prompt, history, selected_model, api_key, image_path=None, is_news=False):
        super().__init__()
        self.prompt = prompt
    ...
    def run(self):
        ...
        self.write_debug_log(f"NEW REQUEST INITIATED (News Mode: {self.is_news})")
        self.write_debug_log(f"Key in use: {masked_key}")
        self.write_debug_log(f"Model selected: '{self.selected_model}'")
        ...
        try:
            config = None
            if self.is_news:
                self.write_debug_log("Enabling google_search grounding tool for News query...")
                config = types.GenerateContentConfig(tools=[{"google_search": {}}])

            self.write_debug_log(f"Calling client.chats.create() with model '{self.selected_model}'...")
            chat = client.chats.create(
                model=self.selected_model,
                history=self.history,
                config=config
            )
            self.write_debug_log("Network call fired (send_message). Awaiting response...")
...
class PromptDialog(QDialog):
    def __init__(self, parent=None, active_font=None, initial_text="", initial_image=None, default_model_idx=3, custom_model=""):
        ...
        if custom_model and custom_model not in model_list:
            model_list.insert(0, custom_model)
            
        self.model_selector.addItems(model_list)
        
        # Robust Selection Strategy: Avoid index shifting issues by targeting the string explicitly
        if custom_model and default_model_idx == 0:
            self.model_selector.setCurrentText(custom_model)
        elif default_model_idx < len(MODELS):
            self.model_selector.setCurrentText(MODELS[default_model_idx])
...
# ============================================================
# CUSTOM UI WIDGET: NEWS / WEB SEARCH DIALOG
# ============================================================
class NewsSearchDialog(QDialog):
    def __init__(self, parent=None, active_font=None, available_keys=None):
        super().__init__(parent)
        self.setWindowTitle("News & Web Search Query")
        self.resize(560, 310)
        ...
        warning_box = QLabel(
            "⚠️ <b>Caution:</b> Search Grounding consumes strict external search quota "
            "(5,000 queries/month free tier). Gemini 3.x models frequently reject search with "
            "immediate 429 errors. Use only <b>Gemini 2.x Flash</b> models here and query sparingly."
        )
        warning_box.setWordWrap(True)
        warning_box.setStyleSheet(
            "background-color: #fff3cd; color: #856404; padding: 8px; border: 1px solid #ffeeba; border-radius: 4px;"
        )
        ...
        self.model_selector = QComboBox()
        self.model_selector.addItems(NEWS_MODELS)
        m_label.setBuddy(self.model_selector)
        row1.addWidget(m_label)
        row1.addWidget(self.model_selector)
        
        k_label = QLabel("&API Key:")
        self.key_selector = QComboBox()
        if available_keys:
            for mask, val in available_keys:
                self.key_selector.addItem(mask, val)
        k_label.setBuddy(self.key_selector)
        row1.addWidget(k_label)
        row1.addWidget(self.key_selector)
        layout.addLayout(row1)
        
        self.input_box = QTextEdit()
        self.input_box.setPlaceholderText("Enter news topic or current events query... (Ctrl+Enter to send)")
        if active_font:
            self.input_box.setFont(active_font)
        layout.addWidget(self.input_box)
        ...
        self.send_btn = QPushButton("🌐 &Search & Ask")
    ...
    def get_data(self):
        return (
            ...
            self.key_selector.currentData(),
            self.new_tab_checkbox.isChecked()
        )
...
class CustomTitleBar(QWidget):
    def __init__(self, parent):
        ...
        self.menu_btn.setToolTip("Menu & Options (Alt+M)")
...
# ============================================================
# CHAT TAB WIDGET (WITH SLEEK IN-TAB FIND BAR)
# ============================================================
class ChatTab(QWidget):
    def __init__(self, parent=None):
        ...
        self.reading_layout = QHBoxLayout(self.reading_container)
        self.reading_layout.setContentsMargins(0, 0, 0, 0)
        self.reading_layout.setSpacing(0)
        ...
        self.reading_layout.addWidget(self.left_spacer)
        self.reading_layout.addWidget(self.browser)
        self.reading_layout.addWidget(self.right_spacer)
        
        self.main_layout.addWidget(self.reading_container)
        
        self.find_bar = QWidget()
        self.find_bar.setFixedHeight(34)
        find_layout = QHBoxLayout(self.find_bar)
        find_layout.setContentsMargins(10, 2, 10, 2)
        find_layout.setSpacing(6)
        
        self.find_input = QLineEdit()
        self.find_input.setPlaceholderText("Find in chat... (Enter for next, Shift+Enter for prev, Esc to close)")
        self.find_input.returnPressed.connect(self.find_next)
        find_layout.addWidget(self.find_input)
        
        self.find_prev_btn = QPushButton("▲ Prev")
        self.find_prev_btn.setFixedWidth(55)
        self.find_prev_btn.clicked.connect(self.find_prev)
        find_layout.addWidget(self.find_prev_btn)
        
        self.find_next_btn = QPushButton("▼ Next")
        self.find_next_btn.setFixedWidth(55)
        ...
        esc_shortcut = QShortcut(QKeySequence("Esc"), self.find_bar)
        esc_shortcut.activated.connect(self.hide_find_bar)
        
        shift_enter = QShortcut(QKeySequence("Shift+Return"), self.find_input)
    ...
    def set_margins(self, margin_pct, center_pct):
        self.reading_layout.setStretch(0, margin_pct)
        self.reading_layout.setStretch(1, center_pct)
        self.reading_layout.setStretch(2, margin_pct)
    ...
    def find_prev(self):
        ...
        if not found:
            ...
            self.browser.setTextCursor(cursor)
            self.browser.find(text, QTextDocument.FindBackward)
...
class GeminiGUI(QWidget):
    def __init__(self):
        ...
        # Default target set to gemini-3.5-flash (Index 3 of MODELS)
    ...
    def init_ui(self):
        ...
        manage_menu = self.main_menu.addMenu("📁 &Manage Tabs (JSON Backup)")
        
        action_export_current = manage_menu.addAction("📦 Export Current Tab")
        action_export_current.triggered.connect(lambda: self.export_tabs('current'))
        
        action_export_all = manage_menu.addAction("📚 Export All Tabs")
        action_export_all.triggered.connect(lambda: self.export_tabs('all'))
        
        action_import = manage_menu.addAction("📥 Import Tabs")
        action_import.triggered.connect(self.import_tabs)

        action_export_docx = self.main_menu.addAction("📝 &Export Word (.doc)")
        action_export_docx.triggered.connect(self.export_docx)
        ...
        action_news = self.main_menu.addAction("🌐 &News / Web Search...")
        action_news.setShortcut("Ctrl+Shift+N")
        action_news.triggered.connect(self.open_news_dialog)
        
        self.main_menu.addSeparator()
        
        action_find = self.main_menu.addAction("🔍 &Find in Chat")
        action_find.setShortcut("Ctrl+F")
        action_find.triggered.connect(self.toggle_find_bar)
        
        action_jump = self.main_menu.addAction("📑 &Jump to Question...")
        action_jump.setShortcut("Ctrl+J")
        action_jump.triggered.connect(self.jump_to_question)
    ...
    # ============================================================
    def export_tabs(self, mode):
        data = []
        date_str = datetime.now().strftime('%Y-%m-%d')
        
        if mode == 'current':
            tab = self.stack.currentWidget()
            if not tab or not tab.current_chat_path or not os.path.exists(tab.current_chat_path):
                QMessageBox.warning(self, "Export", "Current tab is empty or invalid.")
                return
            idx = self.stack.indexOf(tab)
            title = self.title_bar.tab_bar.tabText(idx)
            with open(tab.current_chat_path, "r", encoding="utf-8") as f:
                data.append({"title": title, "markdown": f.read()})
            
            safe_title = "".join(c if c.isalnum() else "_" for c in title).lower()
            default_name = f"BG_Tab_{safe_title}_{date_str}.json"
        else:
            if self.stack.count() == 0:
                QMessageBox.warning(self, "Export", "No tabs to export.")
                return
            for i in range(self.stack.count()):
                tab = self.stack.widget(i)
                if hasattr(tab, "current_chat_path") and tab.current_chat_path and os.path.exists(tab.current_chat_path):
                    title = self.title_bar.tab_bar.tabText(i)
                    with open(tab.current_chat_path, "r", encoding="utf-8") as f:
                        data.append({"title": title, "markdown": f.read()})
            default_name = f"BG_AllTabs_{date_str}.json"

        filepath, _ = QFileDialog.getSaveFileName(self, "Export Tabs", os.path.join(CHAT_DIR, default_name), "JSON Files (*.json);;Text Backup (*.txt)")
        if filepath:
            try:
                with open(filepath, "w", encoding="utf-8") as f:
                    json.dump(data, f, indent=2)
                self.title_bar.status_label.setText("Tabs exported successfully.")
            except Exception as e:
                QMessageBox.critical(self, "Export Error", f"Failed to export: {str(e)}")

    def import_tabs(self):
        filepath, _ = QFileDialog.getOpenFileName(self, "Import Tabs", CHAT_DIR, "Backup Files (*.json *.txt);;All Files (*)")
        if not filepath:
            return
            ...
            for item in data:
                ...
                if "markdown" in item:
                    ...
                    for msg in item["history"]:
                        role = "News/Web Search question" if msg.get("isNews") else ("Question" if msg.get("role") == "user" else "Answer")
                        text = msg.get("text", "")
                ...
                if content:
                    new_filename = datetime.now().strftime("%Y-%m-%d_%H-%M-%S_%f.md")
                    new_filepath = os.path.join(CHAT_DIR, new_filename)
                    
                    with open(new_filepath, "w", encoding="utf-8") as f:
                        f.write(content)
                    
                    self.load_chat_from_path(new_filepath)
                    imported_count += 1
                    
            if imported_count > 0:
                self.title_bar.status_label.setText(f"Successfully imported {imported_count} tab(s).")
            else:
                QMessageBox.warning(self, "Import", "No valid chat data found in file.")
    ...
    def load_persistent_settings(self):
        ...
        self.auto_scroll_enabled = config.get("auto_scroll", False)
        ...
        self.custom_model_input.blockSignals(True)
        self.custom_model_input.setText(self.custom_model)
        self.custom_model_input.blockSignals(False)
    ...
    def setup_settings_dialog(self):
        ...
        self.settings_dialog.resize(340, 290)
        ...
        self.custom_model_input.setPlaceholderText("e.g. gemini-4.0-flash")
        self.custom_model_input.textChanged.connect(self.apply_custom_model)
        form.addRow("Custom Model (Override):", self.custom_model_input)
        ...
        self.auto_scroll_checkbox = QCheckBox("Auto-scroll to bottom on incoming messages")
        self.auto_scroll_checkbox.stateChanged.connect(self.apply_auto_scroll)
        form.addRow("Behavior:", self.auto_scroll_checkbox)
        ...
        self.last_log_label.setStyleSheet("color: #888; font-style: italic; margin-top: 5px;")
    ...
    def apply_custom_model(self):
        self.custom_model = self.custom_model_input.text().strip()
        self.update_config(custom_model=self.custom_model)
                
    def apply_auto_scroll(self, state):
        self.auto_scroll_enabled = (state == Qt.Checked)
        self.update_config(auto_scroll=self.auto_scroll_enabled)
    ...
    # ============================================================
    # IN-TAB SEARCH & NAVIGATION (ZERO CLUTTER)
    # ============================================================
    def toggle_find_bar(self):
        tab = self.stack.currentWidget()
        if not tab: return
        if tab.find_bar.isVisible():
            tab.hide_find_bar()
        else:
            tab.show_find_bar()
    ...
    def jump_to_question(self):
        tab = self.stack.currentWidget()
        if not tab or not tab.current_chat_path or not os.path.exists(tab.current_chat_path):
            QMessageBox.information(self, "Notice", "No questions available in this tab.")
            return
        ...
        for block in content.split("---"):
            has_q = ("## Question" in block) or ("## News/Web Search question" in block)
            if not has_q:
                continue
            try:
                q_header = "## News/Web Search question" if "## News/Web Search question" in block else "## Question"
                user_text = block.split(q_header, 1)[1].split("## Answer", 1)[0].strip()
                
                if "<img" in user_text:
                    user_text = user_text.split("/>", 1)[-1].strip()
                    user_text = user_text.replace("<br>", "").strip()

                if user_text:
                    snippet = user_text.replace('\n', ' ')
                    snippet = snippet if len(snippet) <= 60 else snippet[:60] + "..."
                    
                    search_target = user_text.split('\n')[0].strip()
                    questions_map[snippet] = search_target
            except Exception:
                continue

        if not questions_map:
            QMessageBox.information(self, "Notice", "No questions found in this tab.")
            return

        item, ok = QInputDialog.getItem(self, "Jump to Question", "Select a Question to Jump To:", list(questions_map.keys()), 0, False)
        if ok and item:
            target_text = questions_map[item]
            
            cursor = tab.browser.textCursor()
            cursor.movePosition(QTextCursor.Start)
            tab.browser.setTextCursor(cursor)
            
            found = tab.browser.find(target_text)
            if not found:
                QMessageBox.warning(self, "Error", "Could not jump to that exact line.")
    ...
    def show_help(self):
        help_text = (
            ...
            "• Ctrl+Shift+N : News / Web Search\n"
            "• Ctrl+F : Find Text in Active Tab\n"
            "• Ctrl+J : Jump to Question\n"
            "• Ctrl+T : New Tab\n"
            "• Ctrl+W : Close Tab\n"
            "• Ctrl+Tab : Next Tab\n"
            "• Ctrl+O : Resume Chat\n"
            ...
            "• Alt+Z : Focus Font Size\n"
            "• Alt+W : Focus Reading Width\n"
    ...
    def export_docx(self):
        ...
        if not current_tab or not current_tab.current_chat_path or not os.path.exists(current_tab.current_chat_path):
            QMessageBox.information(self, "Export", "Nothing to export yet.")
            return
            
        base_name = os.path.basename(current_tab.current_chat_path).replace(".md", "")
        default_name = current_tab.current_chat_path.replace(".md", ".doc")
        filepath, _ = QFileDialog.getSaveFileName(self, "Export to Word", default_name, "Word Document (*.doc)")
        
        if filepath:
            try:
                with open(current_tab.current_chat_path, "r", encoding="utf-8") as f:
                    md_text = f.read()
                    
                temp_doc = QTextDocument()
                temp_doc.setMarkdown(md_text)
                clean_html = temp_doc.toHtml()
                ...
                with open(filepath, "w", encoding="utf-8-sig") as f:
                    f.write(word_html)
                self.title_bar.status_label.setText("Word document exported successfully.")
            except Exception as e:
                QMessageBox.critical(self, "Export Error", f"Failed to export: {str(e)}")
    ...
    def parse_markdown_chat(self, path):
        ...
        for block in text.split("---"):
            has_q = ("## Question" in block) or ("## News/Web Search question" in block)
            if not has_q or "## Answer" not in block:
                continue
            try:
                q_header = "## News/Web Search question" if "## News/Web Search question" in block else "## Question"
                user_text = block.split(q_header, 1)[1].split("## Answer", 1)[0].strip()
    ...
    def save_exchange_to_file(self, tab, user_text, gemini_text, image_path=None, is_news=False):
        img_str = f'<img src="{image_path}" width="250" /><br><br>' if image_path else ""
        header = "## News/Web Search question" if is_news else "## Question"
        
        safe_user_text = html.escape(user_text)
        
        with open(tab.current_chat_path, "a", encoding="utf-8") as f:
            f.write(f"{header}\n\n{img_str}{safe_user_text.rstrip()}\n\n")
            f.write(f"## Answer\n\n{gemini_text.rstrip()}\n\n---\n\n")
    ...
    def render_entire_chat(self, tab):
        ...
        current_html = tab.browser.toHtml()
        padding = "<br>" * 50
        tab.browser.setHtml(current_html + padding)
        
        if self.auto_scroll_enabled:
            sb.setValue(sb.maximum())
            QTimer.singleShot(0, lambda: sb.setValue(sb.maximum()))
        else:
            sb.setValue(current_scroll)
            QTimer.singleShot(0, lambda: sb.setValue(current_scroll))
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            if prompt or image_path:
                ...
                if open_in_new_tab:
                    self.new_chat()
                    current_tab = self.stack.currentWidget()
                self.send_message(prompt, image_path, api_key, current_tab, selected_model, is_news=False)
                
            if current_tab:
                current_tab.browser.setFocus()
        else:
    ...
    def open_news_dialog(self):
        ...
        dialog = NewsSearchDialog(self, active_font=self.current_font, available_keys=keys)
        if dialog.exec_() == QDialog.Accepted:
            prompt, selected_model, chosen_key, open_in_new_tab = dialog.get_data()
            if prompt:
                if open_in_new_tab:
                    self.new_chat()
                current_tab = self.stack.currentWidget()
                self.send_message(prompt, None, chosen_key, current_tab, selected_model, is_news=True)
    ...
    def send_message(self, prompt, image_path, api_key, tab, selected_model, is_news=False):
        self.title_bar.prompt_btn.setEnabled(False)
        self.title_bar.status_label.setText("Searching/Thinking..." if is_news else "Thinking...")
        ...
        temp_append = f"\n\n{header}\n\n{img_notice}{safe_prompt}\n\n*Waiting for response...*"
        
        sb = tab.browser.verticalScrollBar()
        current_scroll = sb.value()
        
        tab.browser.setMarkdown(current_md + temp_append)
        
        current_html = tab.browser.toHtml()
        tab.browser.setHtml(current_html + "<br>" * 50)
        ...
        tab.worker = ApiWorker(prompt, tab.history, selected_model, api_key, image_path, is_news=is_news)
        tab.worker.status_update.connect(lambda msg: self.title_bar.status_label.setText(msg))
        tab.worker.finished.connect(
            lambda text, model, t=tab, p=image_path, pr=prompt, k=api_key, n=is_news: 
            self.on_success(pr, text, model, p, t, k, n)
        )
        tab.worker.error.connect(
            lambda err, t=tab, k=api_key, pr=prompt, img=image_path, m=selected_model, n=is_news: 
            self.on_error(err, t, k, pr, img, m, n)
        )
        tab.worker.start()

    def on_success(self, prompt, answer, used_model, image_path, tab, api_key, is_news=False):
        ...
        self.save_exchange_to_file(tab, prompt, answer, image_path, is_news=is_news)

        masked = f"...{api_key[-4:]}" if len(api_key) > 4 else api_key
        log_text = f"Time: {datetime.now().strftime('%H:%M:%S')}\nModel: {used_model} (News: {is_news})\nKey: {masked}"
    ...
    def on_error(self, err_msg, tab, api_key, prompt, image_path, failed_model, is_news=False):
        self.recovered_prompt = prompt
        self.recovered_image = image_path
        
        extra_notice = ""
        if not is_news and ("429" in err_msg or "quota" in err_msg.lower() or "exhausted" in err_msg.lower()):
            ...
            extra_notice = "Search Grounding quota exhausted or restricted. Switch keys or cycle network IP."
        else:

...
class GeminiGUI(QWidget):
    ...
    def jump_to_question(self):
        ...
        questions_map = {}
        # Parse cleanly by split block to isolate just the user's text
        for block in content.split("---"):
            ...
            try:
                ...
                # Strip out any image tags from the search target
                if "<img" in user_text:
                    user_text = user_text.split("/>", 1)[-1].strip()
                    user_text = user_text.replace("<br>", "").strip()

                if user_text:
                    # Create a short readable snippet for the dropdown
                    snippet = user_text.replace('\n', ' ')
                    snippet = snippet if len(snippet) <= 60 else snippet[:60] + "..."
                    
                    # Store the EXACT first line of the question to guarantee a search match
        ...
        if ok and item:
            ...
            # CRITICAL FIX: Instantly snap the invisible search cursor to the absolute top of the document 
            # so it scans from Top -> Bottom. Without this, find() fails if you are already at the bottom.
    ...
    def show_help(self):
        help_text = (
            ...
            "Ergonomics & Behavior:\n"
            "• Scroll-Past-End allows you to scroll the final line to the top of the monitor.\n"
            "• To stop incoming AI responses from pulling you down the page while reading, "
            "uncheck the auto-scroll setting in Preferences.\n\n"
    ...
    def render_entire_chat(self, tab):
        ...
        # CRITICAL FIX: PyQt5 natively strips trailing whitespace from Markdown parsing.
        # We must append raw HTML directly to the body *after* the markdown engine finishes compiling.
    ...
    def send_message(self, prompt, image_path, api_key, tab, selected_model, is_news=False):
        ...
        # Apply padding fix to the waiting screen as well
        current_html = tab.browser.toHtml()

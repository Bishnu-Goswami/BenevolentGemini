...
class PromptDialog(QDialog):
    def __init__(self, parent=None, active_font=None, initial_text="", initial_image=None, default_model_idx=0, custom_model=""):
        super().__init__(parent)
        self.setWindowTitle("New Prompt")
        self.resize(800, 500)  # Massive starting size for heavy logs
        self.setWindowFlags(self.windowFlags() | Qt.WindowMaximizeButtonHint) # Allow full-screen maximizing
        self.image_path = initial_image
        
        layout = QVBoxLayout(self)
        
        self.input_box = QTextEdit()
        self.input_box.setAcceptRichText(False)  # Strips HTML/colors so terminal pastes don't break
        ...
        if custom_model and default_model_idx == 0 and custom_model in model_list:
            self.model_selector.setCurrentIndex(0)
        elif default_model_idx < len(model_list):
...
class GeminiGUI(QWidget):
    ...
    def render_entire_chat(self, tab):
        ...
        if self.auto_scroll_enabled:
            ...
            # Force secondary scroll after Qt layout engine completes
            QTimer.singleShot(0, lambda: sb.setValue(sb.maximum()))
        else:
            sb.setValue(current_scroll)
            # Force lock position after Qt layout engine completes
    ...
    def open_prompt_dialog(self):
        ...
        if dialog.exec_() == QDialog.Accepted:
            ...
            # Save the current_model_index if it's a standard model, otherwise we keep the selection logic flexible
            ...
            # Restore strict keyboard focus to chat window after sending
            if current_tab:
            ...
            # Restore strict keyboard focus to chat window if user cancels
            if current_tab:
                current_tab.browser.setFocus()
    ...
    def send_message(self, prompt, image_path, api_key, tab, selected_model, is_news=False):
        ...
        # Save exact scroll state before appending new text
        sb = tab.browser.verticalScrollBar()
        current_scroll = sb.value()
